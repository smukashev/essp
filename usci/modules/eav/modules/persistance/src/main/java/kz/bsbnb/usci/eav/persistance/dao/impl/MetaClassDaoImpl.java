...
public class MetaClassDaoImpl extends JDBCSupport implements IMetaClassDao {
    ...
    public Select getSimpleSelect(long metaId) {
        ...
        MetaClass metaClass = load(metaId);
        ...
        for (String attribute : names) {
            ...
            if (metaType.isComplex()) {
                select.select(EAV_BE_COMPLEX_VALUES.as("cv" + counter).ENTITY_VALUE_ID
                        .as(metaAttribute.getName().toUpperCase() + "_ID"));
            } else {
                MetaValue metaValue = (MetaValue) metaType;
                switch (metaValue.getTypeCode()) {
                    case STRING:
                        select.select(EAV_BE_STRING_VALUES.as("sv" + counter).VALUE
                                .as(metaAttribute.getName().toUpperCase()));
                        break;
                    case INTEGER:
                        select.select(EAV_BE_INTEGER_VALUES.as("iv" + counter).VALUE
                                .as(metaAttribute.getName().toUpperCase()));
                        break;
                    case BOOLEAN:
                        select.select(EAV_BE_BOOLEAN_VALUES.as("bv" + counter).VALUE
                                .as(metaAttribute.getName().toUpperCase()));
                        break;
                    case DOUBLE:
                        select.select(EAV_BE_DOUBLE_VALUES.as("dov" + counter).VALUE
                                .as(metaAttribute.getName().toUpperCase()));
                        break;
                    case DATE:
                        select.select(EAV_BE_DATE_VALUES.as("dav" + counter).VALUE
                                .as(metaAttribute.getName().toUpperCase()));
                        break;
                    default:
