...
public class DDLHelper {
    ...
    public static void dropDatabase(DataSource dataSource, Database targetModel) {
        Platform platform = PlatformFactory.createNewPlatformInstance(dataSource);
    ...
    public static void main(String[] args) {
        ...
        if (args.length < 2) {
            logger.info("Usage: <command> <filename> [applicationContext]");
            logger.info("Commands: gen, apply");
            return;
        }
        ...
        if (args[0].equals("gen")) {
            ...
            DDLHelper.dropDatabase(source, DDLHelper.readDatabaseFromXML(args[1]));
            DDLHelper.changeDatabase(source, DDLHelper.readDatabaseFromXML(args[1]), false);
        }
    }
}
