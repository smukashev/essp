...
public class ChangeRemainsInterestParser extends BatchParser {
    ...
    @Override
    public void init() {
        currentBaseEntity = new BaseEntity(metaClassRepository.getMetaClass("remains_interest"), batch.getRepDate(), creditorId);
        fieldCurrent = null;
        fieldPastDue = null;
        fieldWriteOf = null;
    }
    ...
    public boolean startElement(XMLEvent event, StartElement startElement, String localName) throws SAXException {
        if (localName.equals("interest")) {
            ...
            BaseEntity baseEntity = new BaseEntity(metaClassRepository.getMetaClass("ref_balance_account"),
                    batch.getRepDate(), creditorId);
            baseEntity.put("no_", new BaseEntityStringValue(0, creditorId, batch.getRepDate(),
                    event.asCharacters().getData(), false, true));
            BaseValue baseValue = new BaseEntityComplexValue(0, creditorId, batch.getRepDate(),
