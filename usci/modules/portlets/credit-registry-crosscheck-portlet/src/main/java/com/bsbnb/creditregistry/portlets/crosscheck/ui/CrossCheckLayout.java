...
import com.bsbnb.creditregistry.portlets.crosscheck.CrossCheckPortletResource;
...
public class CrossCheckLayout extends VerticalLayout {
    ...
    public void attach() {
        ...
        //crossCheckButton
        crossCheckButton = new Button(Localization.RUN_CROSS_CHECK_BUTTON_CAPTION.getValue(), new Button.ClickListener() {
        ...
        Button batchExportToExcel = new Button(Localization.BATCH_EXPORT_TO_EXCEL.getValue(), new Button.ClickListener() {
        ...
        if (businessRulesUrl != null && !businessRulesUrl.isEmpty()) {
            Link businessRulesLink = new Link(Localization.BUSINESS_RULES.getValue(), new ExternalResource(businessRulesUrl));
        ...
        //fileInfoHeaderLabel
        crossCheckHeaderLabel = new Label("<h3>" + Localization.INFO_FILE_TABLE_CAPTION.getValue() + "</h3>", Label.CONTENT_XHTML);
        ...
        exportFileInfoTableButton = new Button(Localization.EXPORT_TABLE_TO_XLS.getValue(), new Button.ClickListener() {
            ...
            public void buttonClick(ClickEvent event) {
                try {
                    byte[] exportBytes = crossCheckTable.exportToXLS(Localization.CROSS_CHECK_EXCEL_CAPTION.getValue(), Localization.CROSS_CHECK_TABLE_COLUMNS.getValue().split(","));
        ...
        messagesHeaderLabel = new Label("<h3>" + Localization.PACKAGE_ERRORS_TABLE_CAPTION.getValue() + "</h3>", Label.CONTENT_XHTML);
        ...
        messagesTable.addListener(new Property.ValueChangeListener() {
            ...
            public void valueChange(ValueChangeEvent event) {
                if (event != null && event.getProperty() != null && event.getProperty().getValue() != null) {
                    CrossCheckMessageDisplayWrapper message = (CrossCheckMessageDisplayWrapper) event.getProperty().getValue();
        ...
        setSpacing(true);
        if ("DEVELOPMENT".equals(viewType)) {
            Label developmentMessageLabel = new Label(Localization.DEVELOPMENT_MESSAGE.getValue(), Label.CONTENT_XHTML);
    ...
    private void exportCrossCheckToExcel(List<Creditor> selectedItems) {
        ...
        List<CrossCheck> crossChecks = provider.getCrossChecks(selectedItems.toArray(new Creditor[0]), (Date) dateField.getValue());
    ...
    private void exportToExcel(List<CrossCheck> uniqueCreditorsCrossChecks) {
        try {
            ...
            WritableFont times12Bold = new jxl.write.WritableFont(jxl.write.WritableFont.TIMES, 12, jxl.write.WritableFont.BOLD);
    ...
    private WritableSheet writeCrossCheckOnSheet(CrossCheck crossCheck, WritableWorkbook workbook) throws WriteException {
        ...
        for (int columnIndex = 0; columnIndex < columnNames.length; columnIndex++) {
            sheet.addCell(new jxl.write.Label(columnIndex, rowCounter, columnNames[columnIndex], times12FormatBold));
        }
        rowCounter++;
        for (CrossCheckMessageDisplayWrapper message : provider.getMessages(crossCheck)) {
            WritableCellFormat usedGroupedCellFormat = message.getIsError() == 0 ? groupedCellFormatGreen : groupedCellFormatRed;
            WritableCellFormat usedCellFormat = message.getIsError() == 0 ? times12FormatGreen : times12FormatRed;
            String description = message.getDescription();
            if (description.length() == 7) {
                description = description.length() == 7 ? description.substring(0, 4) + " " + description.substring(4) : description;
    ...
    private void addNumberCell(WritableSheet sheet, int columnNumber, int rowNumber, String value, WritableCellFormat numberFormat, WritableCellFormat textFormat) throws WriteException {
        Long longValue = null;
        try {
