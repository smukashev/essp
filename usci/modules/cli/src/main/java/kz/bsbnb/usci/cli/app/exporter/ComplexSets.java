...
import static kz.bsbnb.eav.persistance.generated.Tables.EAV_BE_ENTITY_COMPLEX_SETS;


@Component
public class ComplexSets extends AbstractTable {

  ComplexSetValues eavComplexSetValues;

  @Override
  public Table getTable() {
    return EAV_BE_ENTITY_COMPLEX_SETS;
  }

  @PostConstruct
  public void init(){
    eavComplexSetValues = new ComplexSetValues();
    eavComplexSetValues.setDataSource(jdbcTemplate.getDataSource());
    eavComplexSetValues.setContext(context);
  }
  ...
  public Query getQueries(Long entityId) {
    ...
    for(Long setId : sets) {
      Query query = eavComplexSetValues.getQueries(setId);
      qList.addAll(query.getQueries());
      eList.addAll(query.entityList);
    }
