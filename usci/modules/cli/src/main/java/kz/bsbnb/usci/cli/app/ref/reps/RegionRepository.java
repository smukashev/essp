...
public class RegionRepository extends BaseRepository {
    ...
    public RegionRepository() {
        QUERY_ALL = "SELECT * FROM ref.region";
        QUERY_OPEN = "SELECT * FROM ref.region where open_date = to_date('repDate', 'dd.MM.yyyy') " +
                " and (close_date > to_date('repDate','dd.MM.yyyy') or close_date is null)";
        QUERY_CLOSE = "SELECT * FROM ref.region where close_date = to_date('repDate', 'dd.MM.yyyy') and is_last = 1";
        COLUMNS_QUERY = "SELECT * FROM all_tab_cols WHERE owner = 'REF' AND TABLE_NAME='REGION'";
    }
    ...
    public Region[] getByProperty(String key,String value){
