create or replace PROCEDURE "LX_INTER_PASTDUE_CD_RD_FIX" IS
  v_primary_contract_no varchar(255);
  v_primary_contract_date date;
BEGIN

  update LX_INTER_PASTDUE_CD_RD lx set lx.cr_close_date = null, lx.cr_report_date = null, lx.cr_not_found=0 where lx.pastdue_id is not null;

  for pd in (select * from LX_INTER_PASTDUE_CD_RD)
   loop
      declare
      crCreditId number;
      crCloseDate date;
...
--              and attribute_id = 62;
       ...
       EXCEPTION
        WHEN no_data_found THEN
          update LX_INTER_PASTDUE_CD_RD set cr_not_found=1 where pastdue_id=pd.pastdue_id;
...
END LX_INTER_PASTDUE_CD_RD_FIX;