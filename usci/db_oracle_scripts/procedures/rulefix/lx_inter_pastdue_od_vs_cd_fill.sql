create or replace PROCEDURE "LX_INTER_PASTDUE_OD_VS_CD_FILL" IS
    v_primary_contract_no varchar(255);
    v_primary_contract_date date;
    v_creditor_id number;
  BEGIN

    DELETE FROM LX_INTER_PASTDUE_OD_VS_CD;

    --GET DEPT AND PASTDUE
    for pastdues in (
        select pd.entity_id interestId, pd.entity_value_id pastdueId, pd.report_date reportDate, od.value openDate, cd.value closeDate
     ...
     loop
        insert into LX_INTER_PASTDUE_OD_VS_CD(pastdue_id, interest_id, report_date, open_date, close_date)
        values(pastdues.pastdueId, pastdues.interestId, pastdues.reportDate, pastdues.openDate, pastdues.closeDate);
     end loop;
     ...
     for remains in (
        ...
        where ebe.class_id = 43 and clx.entity_value_id in (
            select interest_id  from LX_INTER_PASTDUE_OD_VS_CD
        )
    )
     loop
        update LX_INTER_PASTDUE_OD_VS_CD lx set lx.remains_id=remains.remain where lx.interest_id=remains.interest;
     end loop;
    ...
    for changes in (
        ...
        where ebe.class_id = 57 and clx.entity_value_id in  (
            select remains_id from LX_INTER_PASTDUE_OD_VS_CD
        )
    )
     loop
        update LX_INTER_PASTDUE_OD_VS_CD lx set lx.change_id=changes.chng where lx.remains_id=changes.remain;
     end loop;
    ...
    for credits in (
        ...
        where ebe.class_id = 59 and clx.entity_value_id in (
            select change_id from LX_INTER_PASTDUE_OD_VS_CD
        )
    )
     loop
        update LX_INTER_PASTDUE_OD_VS_CD lx set lx.credit_id=credits.credit where lx.change_id=credits.chng;
     end loop;

    --GET CONTRACT NO and DATE
    for cr in (select * from LX_INTER_PASTDUE_OD_VS_CD)
     loop
  ...
  END LX_INTER_PASTDUE_OD_VS_CD_FILL;