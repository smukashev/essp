create or replace PROCEDURE LX_MATURITY_DATE_MISM_FIX AS
BEGIN

  update LX_MATURITY_DATE_MISM lx set lx.cr_p_cont_date= null, lx.cr_maturity_date = null, lx.cr_not_found=0 where lx.credit_id is not null;

  for credit in (select * from LX_MATURITY_DATE_MISM)
   loop
      declare
      ...
      begin
            ...
            SELECT cr.primary_contract_date, cr.maturity_date
            ...
            UPDATE LX_MATURITY_DATE_MISM lx
          ...
          IF v_cr_maturity_date <> credit.maturity_date THEN
              null;
              --update actual issue date
              --update eav_be_date_values
              --set value=v_cr_a_issue_date
              --where entity_id = cr.credit_id and attribute_id = 159;
          END IF;
