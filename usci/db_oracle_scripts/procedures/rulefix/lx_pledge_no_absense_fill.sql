create or replace PROCEDURE LX_PLEDGE_NO_ABSENSE_FILL
AS
BEGIN

  delete from lx_pledge_no_absense;

  for problems in (

      select be.id pledge_id, plt.entity_value_id pledge_type_id, pl_cont.value pl_contract, plt_code.value pl_type_code
      from eav_be_entities be
        left outer join eav_be_string_values pl_cont
        on pl_cont.entity_id = be.id and pl_cont.attribute_id = 150
  ...
  for pledge in (
    ...
    join lx_pledge_no_absense pl
      on clx.entity_value_id = pl.pledge_id
  )
  loop
    update lx_pledge_no_absense set pledges_id=pledge.pledges_id where pledge_id = pledge.pledge_id;
  end loop;
  ...
  loop
    update lx_pledge_no_absense set credit_id = credit.credit_id where pledges_id = credit.pledges_id;
  end loop;
  ...
  update lx_pledge_no_absense set p_cont_no = p_cont.p_cont_no, p_cont_date = p_cont.p_cont_date,
         creditor_id = p_cont.creditor_id, report_date = p_cont.report_date
  where credit_id = p_cont.credit_id;

  end loop;

END LX_PLEDGE_NO_ABSENSE_FILL;