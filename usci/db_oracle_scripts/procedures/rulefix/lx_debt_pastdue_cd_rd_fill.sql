create or replace PROCEDURE "LX_DEBT_PASTDUE_CD_RD_FILL" IS
  v_primary_contract_no varchar(255);
  v_primary_contract_date date;
BEGIN

  DELETE FROM LX_DEBT_PASTDUE_CD_RD;

  --GET DEPT and PASTDUE
  for pastdues in (
          select pd.entity_id deptId, pd.entity_value_id pastdueId, pd.report_date reportDate,  cd.value closeDate
          ...
          where
            ...
            and pd.entity_value_id not in (309692591,309696994,309696559,309700590,327869493)
  )
   loop
      insert into LX_DEBT_PASTDUE_CD_RD(pastdue_id, dept_id, report_date, close_date)
      values(pastdues.pastdueId, pastdues.deptId, pastdues.reportDate, pastdues.closeDate);
   end loop;
   ...
   for remains in (
      ...
      where ebe.class_id = 43 and clx.entity_value_id in (
          select dept_id  from LX_DEBT_PASTDUE_CD_RD
      )
  )
   loop
      update LX_DEBT_PASTDUE_CD_RD lx set lx.remains_id=remains.remain where lx.dept_id=remains.dept;
   end loop;
  ...
  for changes in (
      ...
      where ebe.class_id = 57 and clx.entity_value_id in  (
          select remains_id from LX_DEBT_PASTDUE_CD_RD
      )
  )
   loop
      update LX_DEBT_PASTDUE_CD_RD lx set lx.change_id=changes.chng where lx.remains_id=changes.remain;
   end loop;
  ...
  for credits in (
      ...
      where ebe.class_id = 59 and clx.entity_value_id in (
          select change_id from LX_DEBT_PASTDUE_CD_RD
      )
  )
   loop
      update LX_DEBT_PASTDUE_CD_RD lx set lx.credit_id=credits.credit where lx.change_id=credits.chng;
   end loop;

  --GET CONTRACT NO and DATE
  for cr in (select * from LX_DEBT_PASTDUE_CD_RD)
   loop
...
END LX_DEBT_PASTDUE_CD_RD_FILL;