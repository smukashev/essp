create or replace PROCEDURE LX_PLEDGE_SUM_ABSENSE_FIX AS
BEGIN

  update LX_PLEDGE_SUM_ABSENSE lx set lx.cr_not_found=0, lx.cr_pl_value=null, lx.cr_pl_type_code=null where lx.credit_id is not null;
  delete from lx_log where rule_name is not null;

  for credit in (select * from LX_PLEDGE_SUM_ABSENSE)
   loop
      begin


            for pledge in(
                  SELECT cr.id credit_id, pl.id id, pl.value_ pl_value,
            ...
            loop
                ...
                update lx_pledge_sum_absense set cr_pl_value = pledge.pl_value where credit_id=credit.credit_id;
                --insert contract_no
                if pledge.pl_value != credit.pl_value then
                    null;
                ...
                update lx_pledge_sum_absense set cr_pl_type_code = pledge.pl_type_code where credit_id=credit.credit_id;
                --update pledge type
      ...
      EXCEPTION
        WHEN no_data_found THEN
          ...
          insert into lx_log(log_date,rule_name,log_message) values(sysdate, 'pledge_sum_absense','MKR credit not foud for ESSP credit_id : '||credit.credit_id);

        WHEN TOO_MANY_ROWS THEN
          insert into lx_log(log_date,rule_name,log_message) values(sysdate, 'pledge_sum_absense','MKR too many debt_remains, may be report_date problem for ESSP credit_id : '||credit.credit_id);
      end;
    end loop;

END LX_PLEDGE_SUM_ABSENSE_FIX;