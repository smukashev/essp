...
create or replace package body DATA_VALIDATION is
  ...
  */
  ...
  begin
    ...
    select round(sum(nvl(dr.value, 0)) / 1000)
      into v_sum
      from reporter.v_credit_his vch, reporter.v_core_remains /*core.debt_remains*/ dr--, ref.shared drt
    ...
    select round(sum(nvl(dr.value, 0)) / 1000)
      into v_sum
      from reporter.v_credit_his /*core.v_credit_his*/ vch, reporter.v_core_remains /*core.debt_remains*/ dr--, ref.shared drt
     where vch.credit_id = dr.credit_id
       --and dr.type_id = drt.id
  ...
  */
  ...
  begin
    select count(1)
      into v_count
      from reporter.v_credit_his vch
  ...
  */
  ...
  begin
    select count(1)
      ...
      from (select vph.contract_no, vph.pledge_type_id
              from reporter.v_credit_his vch, reporter.v_pledge_his vph
  ...
  begin
    ...
    insert into R_TEMP_FS_ZPD
      ...
      select dr.type_code,
             ...
             sum(nvl(dr.value, 0))
        from reporter.v_credit_his   vch,
             reporter.v_core_remains  dr,
             r_ref_balance_account     ba,
             --ref.shared              drt,
             reporter.v_debtor_his        vdh,
             reporter.v_organization_his  voh,
             prod_showcase.r_ref_credit_type    ct
       where vch.creditor_id = g_creditor_id
         and vch.approved = 1
      ...
      for ba in (select vba.no no,
                   ...
                   from v_bal_act vba,
                        ...
                        (select bal.id,
                           ...
                           from reporter.v_core_remains    d,
                                reporter.v_credit_his vch,
                                v_bal_act    bal
                          where vch.creditor_id = g_creditor_id
              ...
              if ba.no = '7130' or ba.no = '6675' or ba.no = '8923' then
                ...
                if ba.no = '7130' then
                  select sum(nvl(dr.value, 0))
                    into v_val_7130
                    from reporter.v_credit_his vch, reporter.v_core_remains dr
                   where vch.creditor_id = g_creditor_id
  ...
  begin
    ...
    for ba in (select substr(vba.no, 1, 4) no,
                      nvl(st.znac, 0) st_sum,
                      nvl(cr.cr_sum, 0) cr_sum
                 from (select distinct substr(no, 1, 4) no
                         from v_bal_act) vba,
                      r_stat_temp st,
    ...
    select round(sum(nvl(pf.value, 0)) / 1000)
     ...
     (select vch.credit_id
             ...
             where vch.approved = 1
               and (vch.portfolio1_id = pf.portfolio_id or vch.portfolio2_id=pf.portfolio_id)
               and (vch.maturity_date >= prev_report_date or
                   vch.maturity_date is null)
  ...
  begin
    select crn.nokbdb_code
      ...
      from r_ref_creditor_nokbdb crn
     where crn.creditor_id = g_creditor_id;
  ...
  begin
      ...
      select sum(nvl(pf.value, 0)), ba.no_, 2, 1
       ...
       where pf.rep_date = g_report_date
         ...
         and exists (select vch.credit_id
                from reporter.v_credit_his vch
               where vch.approved = 1
                 and vch.portfolio_msfo_id = pf.portfolio_id
                 and (vch.maturity_date >= prev_report_date or
                     vch.maturity_date is null)
    ...
    if g_report_date < to_date('01.07.2013', 'dd.mm.yyyy') then
      for empty_port in (select p.code,
                                sum(nvl(pf.value, 0)) as sm
                           from r_core_portfolio_flow_kfn pf, r_ref_portfolio p
                          where pf.rep_date = g_report_date
                            and pf.creditor_id = g_creditor_id
                          ...
                          (select vch.credit_id
                                   from reporter.v_credit_his vch
                                  where vch.approved = 1
                                    and (vch.portfolio1_id = p.ref_portfolio_id or vch.portfolio2_id = p.ref_portfolio_id)
    ...
    for empty_port_msfo in (select p.code,
                                   sum(nvl(pf.value, 0)) as sm
                             ...
                             (select vch.credit_id
                                      from reporter.v_credit_his vch
                                     where vch.approved = 1
                                       and vch.portfolio_msfo_id = p.ref_portfolio_id
                                       and (vch.maturity_date >=
                                           prev_report_date or
                                           vch.maturity_date is null)
  ...
  begin
    ...
    select cross_check_seq.nextval
      into global_cross_check_id
      from dual;
  ...
  begin
    select pu.first_name || ' ' || pu.last_name, pu.email_address
      into user_name, user_email_address
      from r_core_portal_user pu
